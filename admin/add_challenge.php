...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($category_id) {
        ...
        $category_name = $category_row ? $category_row['name'] : '';
    } else {
    ...
    if (!$error) {
        $stmt = $pdo->prepare("
            ...
            (title, category, description, points, file_path, link, flag, tags, category_id, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
        ");

        if ($stmt->execute([$title, $category_name, $description, $points, $file_path, $link, $flag, $tags, $category_id])) {
...
<form action="" method="POST" enctype="multipart/form-data" class="space-y-4 max-w-xl">
  ...
  <div>
    ...
    <input type="text" name="new_category" placeholder="e.g., Web, Crypto" class="w-full px-3 py-2 rounded-md" />
  </div>
  ...
  <div>
    <label>Points</label>
    <input type="number" name="points" value="0" required class="w-full px-3 py-2 rounded-md" />
  </div>
